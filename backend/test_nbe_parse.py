...
nbe : Normalizer = Normalizer()

def parse_file(fname) -> [Expr]:
   with open(fname, "r") as f:
      s = f.read()
      p : Parser = Parser(s)
   return p.parse_file()

def check_file(fname, results):
   es = parse_file(fname)
   ...
   for i in range(len(results)):
      ...
      ans = results[i]
      if v == ans:
         logger.success(f"Good eval for {f1} item {i}: {v}")
      else:
         logger.error(f"Bad evaluation for {f1} item {i} : got {v} expected {ans}")

f1 = "../examples/suite/fundamentals_1.plt"
f1_res = [6, 20, 5, [2,3,-2,5]]
check_file(f1, f1_res)
